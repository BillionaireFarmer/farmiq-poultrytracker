...
const STORAGE_ENTERPRISE = "vafarmer_poultryiq_enterprise";   // persists enterprise selector
...
// ─── Empty batch factory ──────────────────────────────────────────────────────
// `type` is always supplied by selectedEnterprise — never editable by the user.
// This is the single source of truth that locks a batch to its enterprise.
...
const KPI = ({ label, value, sub, accent, danger }) => (
  <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-4 md:p-5 flex flex-col gap-1">
    <p className="text-[10px] md:text-xs font-medium text-slate-500 uppercase tracking-wider leading-tight">{label}</p>
    <p className={`text-lg md:text-2xl font-bold leading-tight ${danger?"text-rose-500":accent?"text-emerald-500":"text-slate-800"}`}>{value}</p>
    {sub && <p className="text-[10px] md:text-xs text-slate-400 leading-tight">{sub}</p>}
  </div>
);
const Card = ({ children, className="" }) => (
  <div className={`bg-white rounded-2xl shadow-sm border border-slate-100 p-4 md:p-6 ${className}`}>{children}</div>
);
...
function Dashboard() {
  ...
  // Auto-collapse sidebar on mobile; set up resize listener
  ...
  // ── 4. Derived: enterprise-scoped batch list ───────────────────────────────
  // All pages read from filteredBatches — no cross-enterprise data ever leaks.
  const filteredBatches = useMemo(
    () => batches.filter(b => b.farm.type === selectedEnterprise),
    [batches, selectedEnterprise]
  );

  // ── 5. Derived: active batch — always within enterprise scope ─────────────
  const activeBatch = useMemo(
    () => filteredBatches.find(b => b.id === activeBatchId) ?? filteredBatches[0] ?? null,
    [filteredBatches, activeBatchId]
  );

  // ── 6. Reactive enterprise switch ─────────────────────────────────────────
  // Runs when enterprise OR batches changes.
  // Rule: keep current selection if it's still valid; otherwise auto-select
  // the first matching batch; if none exist, set null.
  // Early-return prevents unnecessary setState when nothing needs to change.
  useEffect(() => {
    const currentIsValid = batches.some(
      b => b.id === activeBatchId && b.farm.type === selectedEnterprise
    );
    if (currentIsValid) return;
    const first = batches.find(b => b.farm.type === selectedEnterprise);
    setActiveBatchId(first?.id ?? null);
  }, [selectedEnterprise, batches]); // eslint-disable-line react-hooks/exhaustive-deps

  const isClosed = activeBatch?.status === "closed";

  const calc = useMemo(
    () => activeBatch ? calcBatch(activeBatch) : null,
    [activeBatch]
  );

  // ── 7. Batch mutators — all guarded against null activeBatch ──────────────
  // useCallback keeps function identity stable so child components don't
  // re-render when unrelated state changes.

  const updateBatch = useCallback(
    (id, updater) => setBatches(p => p.map(b => b.id === id ? updater(b) : b)),
    []
  );

  // patchFarm blocks `type` mutations — farm.type is immutable after creation.
  // This is the defensive code-level enforcement of enterprise data integrity.
  const patchFarm = useCallback((k, v) => {
    if (!activeBatch) return;
    if (k === "type") return;  // integrity guard: type is locked at creation
    updateBatch(activeBatch.id, b => ({ ...b, farm: { ...b.farm, [k]: v } }));
  }, [activeBatch, updateBatch]);

  const patchBroilerRev = useCallback((k, v) => {
    if (!activeBatch) return;
    updateBatch(activeBatch.id, b => ({ ...b, broilerRev: { ...b.broilerRev, [k]: v } }));
  }, [activeBatch, updateBatch]);

  const patchLayerRev = useCallback((k, v) => {
    if (!activeBatch) return;
    updateBatch(activeBatch.id, b => ({ ...b, layerRev: { ...b.layerRev, [k]: v } }));
  }, [activeBatch, updateBatch]);

  const patchMortality = useCallback((v) => {
    if (!activeBatch) return;
    updateBatch(activeBatch.id, b => ({
      ...b, mortality: Math.min(Number(v) || 0, b.farm.birdsStocked)
    }));
  }, [activeBatch, updateBatch]);

  const patchFeedLog = useCallback((fn) => {
    if (!activeBatch) return;
    updateBatch(activeBatch.id, b => ({ ...b, feedLog: fn(b.feedLog) }));
  }, [activeBatch, updateBatch]);

  const patchExpenses = useCallback((fn) => {
    if (!activeBatch) return;
    updateBatch(activeBatch.id, b => ({ ...b, expenses: fn(b.expenses) }));
  }, [activeBatch, updateBatch]);

  // ── 8. Batch lifecycle ─────────────────────────────────────────────────────
  // New batches always inherit selectedEnterprise — farm.type is never user-input.
  const createBatch = useCallback(() => {
    const name = newBatchName.trim() || `Batch ${batches.length + 1}`;
    const nb = emptyBatch(name, selectedEnterprise);
    setBatches(p => [...p, nb]);
    setActiveBatchId(nb.id);
    setNewBatchName("");
    setShowNewBatch(false);
    setPage("overview");
  }, [newBatchName, batches.length, selectedEnterprise]);

  const closeBatch = useCallback(() => {
    if (!activeBatch) return;
    if (!window.confirm(`Close "${activeBatch.name}"? It will become read-only.`)) return;
    updateBatch(activeBatch.id, b => ({ ...b, status: "closed" }));
  }, [activeBatch, updateBatch]);

  const reopenBatch = useCallback(() => {
    if (!activeBatch) return;
    updateBatch(activeBatch.id, b => ({ ...b, status: "active" }));
  }, [activeBatch, updateBatch]);
  ...
  if (!activeBatch || !calc) {
    const enterpriseLabel = selectedEnterprise === "broiler" ? "Broiler" : "Layer";
    const icon            = selectedEnterprise === "broiler" ? "🐔" : "🥚";
    const otherEnterprise = selectedEnterprise === "broiler" ? "layer" : "broiler";
    const otherLabel      = selectedEnterprise === "broiler" ? "🥚 Layer" : "🐔 Broiler";
    const isGloballyEmpty = batches.length === 0;
    return (
        ...
        <div className="text-center max-w-sm px-6">
          ...
          <h2 className="text-xl font-bold text-slate-800 mb-2">
            {isGloballyEmpty
              ...
              : `No ${enterpriseLabel} batches found`}
          </h2>
          <p className="text-sm text-slate-400 mb-6">
            {isGloballyEmpty
              ? "Track feed, expenses, revenue and profitability for your flock."
              : `Switch enterprise or create a new ${enterpriseLabel} batch to get started.`}
          </p>
          <div className="flex flex-col gap-3 items-center">
            <button onClick={createBatch}
              ...
              <IcPlus/> Create {enterpriseLabel} Batch
            </button>
            {!isGloballyEmpty && (
              <button onClick={() => setSelectedEnterprise(otherEnterprise)}
  ...
  return (
    <div className="flex h-screen bg-slate-50 font-sans">
      ...
      <aside className={`
        ...
        ${sidebarOpen ? "w-56" : isMobile ? "w-0 overflow-hidden" : "w-16"}
        bg-slate-900 flex flex-col transition-all duration-200
      `}>
        <div className="flex items-center gap-3 px-4 py-5 border-b border-slate-700">
          ...
          <button onClick={()=>setSidebarOpen(p=>!p)} className="ml-auto text-slate-400 hover:text-white p-1">
            <IcChevron dir={sidebarOpen?"left":"right"}/>
          </button>
        </div>
        <nav className="flex-1 py-4 space-y-1 px-2">
          {nav.map(({id,label,Icon:Ic})=>(
            <button key={id} onClick={()=>{ setPage(id); if(isMobile) setSidebarOpen(false); }}
      ...
      <main className="flex-1 overflow-y-auto overflow-x-hidden flex flex-col min-w-0">
        <header className="sticky top-0 z-10 bg-white border-b border-slate-100 px-4 md:px-6 py-3 flex flex-col md:flex-row md:items-center gap-2 md:gap-4">
          {/* Row 1 on mobile: hamburger + selectors + status badges */}
          <div className="flex items-center gap-2 flex-1 min-w-0 flex-wrap">
            {/* Hamburger — always visible, opens sidebar */}
            <button onClick={()=>setSidebarOpen(p=>!p)}
              className="flex-shrink-0 text-slate-500 hover:text-slate-800 p-1.5 rounded-lg hover:bg-slate-100 transition-colors">
              <svg width={18} height={18} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round">
                <path d="M3 12h18M3 6h18M3 18h18"/>
              </svg>
            </button>
            {/* ── Global enterprise selector — master switch for the entire app ── */}
            <select value={selectedEnterprise}
              onChange={e => setSelectedEnterprise(e.target.value)}
              className="border border-slate-300 rounded-xl px-2.5 py-1.5 text-xs md:text-sm font-semibold text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-300 bg-white flex-shrink-0">
              <option value="broiler">🐔 Broiler</option>
              <option value="layer">🥚 Layer</option>
            </select>
            {/* ── Batch selector — scoped to current enterprise only ── */}
            <select value={activeBatchId ?? ""}
              onChange={e=>{setActiveBatchId(e.target.value);setPage("overview");}}
              className="border border-slate-200 rounded-xl px-2.5 py-1.5 text-xs md:text-sm font-semibold text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-300 bg-white min-w-0 max-w-[140px] md:max-w-[200px] truncate">
              {filteredBatches.map(b=><option key={b.id} value={b.id}>{b.name}{b.status==="closed"?" 🔒":""}</option>)}
            </select>
            {isClosed && <span className="flex-shrink-0 flex items-center gap-1 text-xs text-amber-600 bg-amber-50 border border-amber-200 px-2 py-1 rounded-lg"><IcLock/> Closed</span>}
            {!isClosed && <button onClick={closeBatch} className="flex-shrink-0 flex items-center gap-1 text-xs text-slate-500 hover:text-amber-600 border border-slate-200 hover:border-amber-300 px-2 py-1 rounded-lg transition-colors whitespace-nowrap"><IcLock/> Close</button>}
            {isClosed  && <button onClick={reopenBatch} className="flex-shrink-0 text-xs text-indigo-600 border border-indigo-200 px-2 py-1 rounded-lg hover:bg-indigo-50 whitespace-nowrap">Reopen</button>}
          </div>
          {/* Row 2 on mobile: new batch + status pill */}
            ...
            {showNewBatch ? (
              <>
                <input value={newBatchName} onChange={e=>setNewBatchName(e.target.value)}
                  ...
                  className="border border-slate-200 rounded-xl px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300 w-28 md:w-36"/>
                <Btn onClick={createBatch} small>Create</Btn>
                <Btn onClick={()=>setShowNewBatch(false)} variant="ghost" small>✕</Btn>
              </>
            ) : (
              <button onClick={()=>setShowNewBatch(true)}
                className="flex items-center gap-1.5 text-xs font-medium text-indigo-600 border border-indigo-200 hover:bg-indigo-50 px-3 py-1.5 rounded-xl transition-colors whitespace-nowrap">
                <IcPlus/> New Batch
              </button>
            )}
            {calc.mortalityRate>10 && <span className="hidden sm:inline-flex text-xs font-semibold px-2.5 py-1 rounded-full bg-rose-100 text-rose-600 whitespace-nowrap">🚨 High Mortality</span>}
            <span className={`text-xs font-semibold px-2.5 py-1 rounded-full whitespace-nowrap ${profitable?"bg-emerald-50 text-emerald-600":"bg-rose-50 text-rose-600"}`}>
        ...
        <div className="px-4 md:px-8 py-4 md:py-6 flex-1">
          <div className="mb-4 md:mb-5 flex items-center justify-between gap-3">
            <div className="min-w-0">
              <h1 className="text-lg md:text-xl font-bold text-slate-800 truncate">{nav.find(n=>n.id===page)?.label}</h1>
              <p className="text-[10px] md:text-xs text-slate-400 truncate">{activeBatch.name} · {activeBatch.farm.type==="broiler"?"🐓 Broiler":"🥚 Layer"} · {calc.aliveBirds.toLocaleString()} alive · Age {calc.ageInWeeks}wk</p>
            </div>
            {/* PDF export shortcut on profit page */}
            {page==="profit" && (
              <Btn variant="success" onClick={()=>exportPDF(activeBatch, calc)}>
                <IcDownload/> <span className="hidden sm:inline">Export Investor Report</span><span className="sm:hidden">PDF</span>
              </Btn>
            )}
          </div>
          ...
          {page==="overview" && (
            <>
              ...
              <div className="grid md:grid-cols-2 gap-4 md:gap-6 mb-5 md:mb-6">
                <Card>
                  <h3 className="text-sm font-semibold text-slate-600 mb-3 md:mb-4">Daily Feed Usage (kg)</h3>
                ...
                <Card>
                  <h3 className="text-sm font-semibold text-slate-600 mb-3 md:mb-4">Cost Breakdown</h3>
                  <ResponsiveContainer width="100%" height={200}>
                    <PieChart>
          ...
          {page==="setup"    && <SetupPage batch={activeBatch} calc={calc} isClosed={isClosed} patchFarm={patchFarm} patchMortality={patchMortality} updateBatch={updateBatch} onReset={resetAllData} selectedEnterprise={selectedEnterprise}/>}
          {page==="comparison" && <BatchComparison batches={filteredBatches} activeBatchId={activeBatchId} onSwitch={id=>{setActiveBatchId(id);setPage("overview");}}/>}
        </div>
      </main>
    </div>
  );
    ...
    <>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 mb-5 md:mb-6">
        ...
        {batch.farm.type==="broiler" && (
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-4 md:p-5 flex flex-col gap-1">
            <p className="text-[10px] md:text-xs font-medium text-slate-500 uppercase tracking-wider">FCR (Live Weight)</p>
            <div className="flex items-center gap-2">
              <p className="text-lg md:text-2xl font-bold text-slate-800">{calc.fcr}</p>
            ...
            <p className="text-[10px] md:text-xs text-slate-400">Biomass: {fmt(calc.totalBiomass,1)} kg</p>
          </div>
        )}
      </div>
      {batch.farm.type==="broiler" && (
        <Card className="mb-4 md:mb-6">
          <h3 className="text-sm font-semibold text-slate-600 mb-3 md:mb-4">FCR Inputs — Current Live Weight</h3>
      ...
      <div className="grid md:grid-cols-2 gap-4 md:gap-6 mb-5 md:mb-6">
        ...
        <Card>
          <h3 className="text-sm font-semibold text-slate-600 mb-3 md:mb-4">Daily Feed Cost (₦)</h3>
          <ResponsiveContainer width="100%" height={200}>
      ...
      <Card>
        <h3 className="text-sm font-semibold text-slate-600 mb-3 md:mb-4">Log Feed Entry</h3>
        ...
        <div className="overflow-x-auto -mx-1">
          <table className="w-full text-xs md:text-sm min-w-[360px]">
            <thead><tr className="border-b border-slate-100">
              {["Date","Feed (kg)","Cost (₦)","₦/kg",""].map(h=>(
                <th key={h} className="text-left text-xs font-medium text-slate-500 pb-2 pr-3 md:pr-4">{h}</th>
              ))}
            </tr></thead>
            <tbody>
              {[...batch.feedLog].reverse().map(r=>(
                <tr key={r.id} className="border-b border-slate-50 hover:bg-slate-50">
                  <td className="py-2 pr-3 md:pr-4">{r.date}</td>
                  <td className="py-2 pr-3 md:pr-4">{r.kgUsed}</td>
                  <td className="py-2 pr-3 md:pr-4">₦{fmt(r.cost)}</td>
                  <td className="py-2 pr-3 md:pr-4">₦{fmtN(r.cost/r.kgUsed)}</td>
                  <td className="py-2">{!isClosed&&<Btn onClick={()=>del(r.id)} variant="danger" small>✕</Btn>}</td>
                </tr>
              ))}
            </tbody>
          </table>
  ...
  return (
    <>
      ...
      <div className="grid md:grid-cols-2 gap-4 md:gap-6 mb-5 md:mb-6">
        ...
        <Card>
          <h3 className="text-sm font-semibold text-slate-600 mb-3 md:mb-4">Add Expense</h3>
      ...
      <Card>
        <h3 className="text-sm font-semibold text-slate-600 mb-3 md:mb-4">Expense Log</h3>
        <div className="overflow-x-auto -mx-1">
            ...
            <tbody>
              {batch.expenses.map(e=>(
                <tr key={e.id} className="border-b border-slate-50 hover:bg-slate-50">
                  <td className="py-2 pr-3 md:pr-4"><span className="px-2 py-0.5 rounded-lg text-xs font-medium bg-indigo-50 text-indigo-700">{e.category}</span></td>
                  <td className="py-2 pr-3 md:pr-4 font-medium">₦{fmt(e.amount)}</td>
                  <td className="py-2 pr-3 md:pr-4 text-slate-500 max-w-[120px] truncate">{e.note}</td>
                  <td className="py-2">{!isClosed&&<Btn onClick={()=>del(e.id)} variant="danger" small>✕</Btn>}</td>
                </tr>
              ))}
            </tbody>
          </table>
  ...
  return (
    <>
      ...
      {batch.farm.type==="broiler" && calc.daysToMarket>0 && (
        <div className="bg-amber-50 border border-amber-200 rounded-2xl px-4 md:px-5 py-3 mb-5 text-xs text-amber-700">
      ...
      {batch.farm.type==="layer" && (
        <div className="bg-indigo-50 border border-indigo-100 rounded-2xl px-4 md:px-5 py-3 mb-5 text-xs text-indigo-700">
      ...
      <div className="grid md:grid-cols-2 gap-4 md:gap-6">
        <Card>
          <h3 className="text-sm font-semibold text-slate-600 mb-3 md:mb-4">Revenue vs Cost</h3>
          <ResponsiveContainer width="100%" height={220}>
        ...
        <Card>
          <h3 className="text-sm font-semibold text-slate-600 mb-3 md:mb-4">Revenue Settings</h3>
          {batch.farm.type==="broiler" ? (
            <div className="flex flex-col gap-3">
              ...
              <div className="mt-2 bg-emerald-50 rounded-xl p-3 md:p-4">
                <p className="text-xs text-slate-500">Revenue per bird</p>
                <p className="text-xl md:text-2xl font-bold text-emerald-600">₦{fmt(broilerPerBird)}</p>
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="mt-2 bg-emerald-50 rounded-xl p-3 md:p-4">
                ...
                <p className="text-xl md:text-2xl font-bold text-emerald-600">₦{fmt(calc.layerRevenueCumulative)}</p>
              </div>
            </div>
          )}
        </Card>
...
function ProfitPage({ batch, calc, onExport }) {
  ...
  return (
    <>
      <div className={`rounded-2xl p-4 md:p-6 mb-5 md:mb-6 ${profitable?"bg-emerald-50 border border-emerald-200":"bg-rose-50 border border-rose-200"}`}>
        <div className="flex items-center justify-between flex-wrap gap-3 md:gap-4">
          <div className="flex items-center gap-3 md:gap-4">
            <div className={`text-3xl md:text-5xl font-black ${profitable?"text-emerald-600":"text-rose-600"}`}>{profitable?"▲":"▼"}</div>
            <div>
              ...
              <p className={`text-2xl md:text-4xl font-black ${profitable?"text-emerald-700":"text-rose-700"}`}>₦{fmt(Math.abs(calc.grossProfit))}</p>
              <p className="text-xs md:text-sm text-slate-500 mt-1">Margin: {calc.margin}% · Per bird: ₦{fmt(calc.profitPerBird)}
                {batch.farm.type==="layer"&&<span className="ml-2 text-indigo-500 text-xs">(Cumulative)</span>}
              </p>
            </div>
          </div>
      ...
      <div className="grid md:grid-cols-2 gap-4 md:gap-6">
        <Card>
          <h3 className="text-sm font-semibold text-slate-600 mb-3 md:mb-4">Cost Waterfall</h3>
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={[
              { name:"Chick Cost", value:calc.chickCost },
              { name:"Feed",       value:calc.totalFeedCost },
        ...
        <Card>
          <h3 className="text-sm font-semibold text-slate-600 mb-3 md:mb-4">Break-Even Analysis</h3>
          <div className="space-y-3 md:space-y-4">
            {batch.farm.type==="broiler"&&calc.breakEvenKg&&(
              <div className="bg-indigo-50 rounded-xl p-3 md:p-4">
                <p className="text-xs text-slate-500 mb-1">Break-even price per kg</p>
                <p className="text-2xl md:text-3xl font-bold text-indigo-600">₦{fmt(calc.breakEvenKg)}</p>
            ...
            {batch.farm.type==="layer"&&calc.breakEvenCrate&&(
              <div className="bg-indigo-50 rounded-xl p-3 md:p-4">
                ...
                <p className="text-2xl md:text-3xl font-bold text-indigo-600">₦{fmt(calc.breakEvenCrate)}</p>
...
// ═════════════════════════════════════════════════════════════════════════════
// SETUP PAGE
// farm.type is intentionally NOT editable here.  It is locked at batch-creation
// time to match the enterprise context.  The header enterprise selector is the
// only way to switch context.  patchFarm already blocks type mutations in code.
// ═════════════════════════════════════════════════════════════════════════════
function SetupPage({ batch, calc, isClosed, patchFarm, patchMortality, updateBatch, onReset, selectedEnterprise }) {
  return (
    <>
      {/* Enterprise context — informational only, not interactive */}
      <div className="mb-4 md:mb-5 px-4 py-3 bg-indigo-50 border border-indigo-100 rounded-xl text-xs text-indigo-700 flex items-center gap-2">
        <span className="text-base">{selectedEnterprise === "broiler" ? "🐔" : "🥚"}</span>
        <span>
          <strong>Enterprise:</strong> {selectedEnterprise === "broiler" ? "Broiler" : "Layer"}
          {" "}— To switch enterprise, use the selector in the top header bar.
        </span>
      </div>
      ...
      {Number(calc.mortalityRate)>10&&<div className="mb-3 md:mb-4"><AlertCard type="danger" title="Critical Mortality Alert" message={`Mortality at ${calc.mortalityRate}% exceeds 10% threshold.`}/></div>}
      {Number(calc.mortalityRate)>5&&Number(calc.mortalityRate)<=10&&<div className="mb-3 md:mb-4"><AlertCard type="warning" title="Elevated Mortality" message={`Mortality at ${calc.mortalityRate}% above 5% warning threshold.`}/></div>}

      <div className="grid md:grid-cols-2 gap-4 md:gap-6 mb-4 md:mb-6">
        <Card>
          <h3 className="text-sm font-semibold text-slate-600 mb-3 md:mb-4">Farm Configuration</h3>
          <div className="grid grid-cols-1 gap-4">
            <Input label="Batch Name" value={batch.name} disabled={isClosed} onChange={v=>updateBatch(batch.id,b=>({...b,name:v}))}/>
            {/* Farm Type is read-only — immutable after creation */}
            <div className="flex flex-col gap-1">
              <label className="text-xs font-medium text-slate-500 uppercase tracking-wider">Farm Type</label>
              <div className="border border-slate-200 rounded-xl px-3 py-2 text-sm bg-slate-50 text-slate-400">
                {batch.farm.type === "broiler" ? "🐓 Broiler" : "🥚 Layer"}
                <span className="ml-2 text-xs text-slate-300">(set at creation)</span>
              </div>
            </div>
        ...
        <Card>
          <h3 className="text-sm font-semibold text-slate-600 mb-3 md:mb-4">Cost Parameters</h3>
          <div className="grid grid-cols-1 gap-4">
    ...
    <>
      ...
      <Card>
          ...
          <table className="w-full text-xs md:text-sm min-w-[640px]">
            <thead>
              <tr className="border-b-2 border-slate-100">
                ...
                <SortTh col="mortalityPct"  label="Mort %"/>
                <SortTh col="grandExpenses" label="Cost"/>
                <SortTh col="totalRevenue"  label="Revenue"/>
                <SortTh col="grossProfit"   label="Profit"/>
                <SortTh col="margin"        label="Margin"/>
                <SortTh col="profitPerBird" label="₦/Bird"/>
                <th className="text-left text-xs font-medium text-slate-500 pb-3 pr-3">Status</th>
                <th className="text-left text-xs font-medium text-slate-500 pb-3">PDF</th>
              </tr>
            </thead>
            <tbody>
              {sorted.map(r=>{
                ...
                return (
                  <tr key={r.id} onClick={()=>onSwitch(r.id)}
                    ...
                    <td className="py-2.5 pr-3">
                      <span className={`px-1.5 py-0.5 rounded-lg text-xs font-medium ${r.type==="broiler"?"bg-orange-50 text-orange-700":"bg-yellow-50 text-yellow-700"}`}>
                        {r.type==="broiler"?"🐓":"🥚"} {r.type}
                      </span>
                    </td>
                    <td className="py-2.5 pr-3 text-slate-600">{r.ageInWeeks}</td>
                    <td className="py-2.5 pr-3 text-slate-600">{r.birdsStocked.toLocaleString()}</td>
                    <td className={`py-2.5 pr-3 font-medium ${r.mortalityPct>10?"text-rose-600":r.mortalityPct>5?"text-amber-600":"text-slate-600"}`}>{r.mortalityPct.toFixed(1)}%</td>
                    <td className="py-2.5 pr-3 text-slate-700">₦{fmt(r.grandExpenses)}</td>
                    <td className="py-2.5 pr-3 text-slate-700">₦{fmt(r.totalRevenue)}</td>
                    <td className={`py-2.5 pr-3 font-bold ${r.grossProfit>=0?"text-emerald-600":"text-rose-600"}`}>
                      {r.grossProfit>=0?"":"−"}₦{fmt(Math.abs(r.grossProfit))}
                    </td>
                    <td className={`py-2.5 pr-3 font-medium ${r.margin>=0?"text-emerald-600":"text-rose-600"}`}>{r.margin.toFixed(1)}%</td>
                    <td className={`py-2.5 pr-3 font-medium ${r.profitPerBird>=0?"text-emerald-600":"text-rose-600"}`}>₦{fmt(r.profitPerBird)}</td>
                    <td className="py-2.5 pr-3">
                      <span className={`px-1.5 py-0.5 rounded-lg text-xs font-medium ${r.status==="closed"?"bg-slate-100 text-slate-500":"bg-green-50 text-green-700"}`}>
                        {r.status==="closed"?"🔒":"●"}
                      </span>
                    </td>
                    <td className="py-2.5" onClick={e=>e.stopPropagation()}>
                      <button onClick={()=>exportPDF(r._batch, r._calc)}
                        className="flex items-center gap-1 text-xs text-indigo-600 border border-indigo-200 hover:bg-indigo-50 px-2 py-1 rounded-lg transition-colors">
                        <IcDownload/>
                      </button>
                    </td>
                  </tr>
        ...
        <div className="mt-6 md:mt-8">
          <h4 className="text-sm font-semibold text-slate-600 mb-3 md:mb-4">Profit Comparison</h4>
          <ResponsiveContainer width="100%" height={200}>
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center px-4 font-sans">
      <div className="w-full max-w-sm">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 md:p-8">
          ...
          {notice && (
            <div className="mb-4 bg-emerald-50 border border-emerald-200 text-emerald-700 text-xs rounded-xl px-4 py-3">{notice}</div>
          )}
          {error && (
            <div className="mb-4 bg-rose-50 border border-rose-200 text-rose-600 text-xs rounded-xl px-4 py-3">{error}</div>
          )}
